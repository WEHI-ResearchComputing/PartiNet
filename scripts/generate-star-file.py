...
def yolo_to_starfile(yolo_coords, image_width, image_height):
...
def generate_output(labels, filename, star_writer):
    for index, row in labels.iterrows():
        # x_centre = row['X-Coordinate']
        # y_centre = row['Y-Coordinate']
        # diameter = row['Diameter']

        x_centre, y_centre, diameter = yolo_to_starfile(row, img_width, img_height)
...
with open(star_out_path, "w") as star_file:
    ...
    for image in os.listdir(images_path):
        ...
        generate_output(labels, star_filename, star_writer)
